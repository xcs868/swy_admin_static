<template>
  <div>
    <a-card :bordered="false">
      <a-button v-auth="['gift:poolGift:save']" type="primary" @click="addGift">添加礼物</a-button>
      <a-table :columns="columns" :data="tableData" stripe :pagination="false" :scroll="{ x: 2000 }" :bordered="{ cell:true }" class="mt-5">
        ...
        <template #operation="{ record }">
          ...
          >
            <a-link v-auth="['gift:poolGift:update']">{{ record.gift_state === '1' ? "下架" : "上架" }}</a-link>
          </a-popconfirm>
          <a-link v-auth="['gift:poolGift:update']" @click="handleEdit(record)">编辑</a-link>
        </template>
      </a-table>
    </a-card>
    <a-modal
...
import poolGift from '@/api/gift/poolGift'
...
const addSubmit = async () => {
  ...
  const res = await poolGift.save(addForm.value)
  if (res.success) {
...
const handleGrounding = async (record) => {
  ...
  const res = await poolGift.putAway(data)
  if (res.success) {
    Message.success(res.message)
    getList()
  }
...
const editSubmit = async () => {
  ...
  const res = await poolGift.update(data)
  if (res.success) {
    Message.success(res.message)
    getList()
  }
...
const handleForecast = async (record) => {
  ...
  const res = await poolGift.read(record.gift_id, {})
  if (res.success) {
...
const getList = async () => {
  const res = await poolGift.getList()
  if (res.success) {
...
<script>
// export default { name: 'gift:poolGift' }
</script>
