  ...
  <div>
    ...
    <a-card title="配置信息" :bordered="false" class="mt-2.5">
      ...
      <div class="mt-2.5 text-right">
        <a-button v-auth="['gift:poolReservoir:update']" type="primary" @click="releaseConfig">发布配置</a-button>
      </div>
    </a-card>
  </div>
</template>
...
import poolReservoir from '@/api/gift/poolReservoir'
...
const releaseConfig = () => {
  Modal.info({
    ...
    onOk: async () => {
      ...
      const res = await poolReservoir.update({ data })
      if (res.success) {
...
const getList = async () => {
  const res = await poolReservoir.getList()
  if (res.success) {
...
<script>
// export default { name: 'gift:poolReservoir' }
</script>
